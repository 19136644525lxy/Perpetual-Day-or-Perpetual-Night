...
配置文件缺失或损坏时会**自动重新生成**完整默认配置。

当文件缺少某些配置项（典型情况：升级模组后新增了字段）时，模组会**合并补全**：
新增项以默认值写入文件，**你已修改过的值一律保留、绝不覆盖**。
补齐过的项会打印在日志中（`已补齐缺失配置项: [...]`），便于确认本次升级引入了哪些新配置。

> 实现说明：合并以「文件的实际 JSON 内容」为基准逐键比对，而不是重新序列化配置对象 ——
> 后者会让文件中没有对应字段的键、以及你手写的取值被默认值覆盖。
> `configVersion` 属于模组管理的元数据，会随合并自动提升到当前版本。
