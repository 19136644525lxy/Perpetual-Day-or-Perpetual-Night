...
implements ChunkHolder.PlayersWatchingChunkProvider {
    ...
    public static boolean isWithinDistance(int x1, int z1, int x2, int z2, int distance) {
        ...
        long m = Math.min(i, j);
        long n = m * m + l * l;
        return n < (long)(k = distance * distance);
    }
    ...
    private static boolean isOnDistanceEdge(int x1, int z1, int x2, int z2, int distance) {
        ...
        return !ThreadedAnvilChunkStorage.isWithinDistance(x1 + 1, z1 + 1, x2, z2, distance) || !ThreadedAnvilChunkStorage.isWithinDistance(x1 - 1, z1 + 1, x2, z2, distance) || !ThreadedAnvilChunkStorage.isWithinDistance(x1 + 1, z1 - 1, x2, z2, distance) || !ThreadedAnvilChunkStorage.isWithinDistance(x1 - 1, z1 - 1, x2, z2, distance);
    }
    ...
    public CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> getChunk(ChunkHolder holder, ChunkStatus requiredStatus) {
        ...
        if (!requiredStatus.shouldAlwaysUpgrade() && (optional = holder.getChunkAt(requiredStatus.getPrevious(), this).getNow(ChunkHolder.UNLOADED_CHUNK).left()).isPresent() && optional.get().getStatus().isAtLeast(requiredStatus)) {
    ...
    private CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> upgradeChunk(ChunkHolder holder, ChunkStatus requiredStatus) {
        ...
        this.world.getProfiler().visit(() -> "chunkGenerate " + requiredStatus);
    ...
    protected void releaseLightTicket(ChunkPos pos) {
        this.mainThreadExecutor.send(Util.debugRunnable(() -> this.ticketManager.removeTicketWithLevel(ChunkTicketType.LIGHT, pos, ChunkLevels.getLevelFromStatus(ChunkStatus.LIGHT), pos), () -> "release light ticket " + pos));
    }
    ...
    public CompletableFuture<Either<WorldChunk, ChunkHolder.Unloaded>> makeChunkTickable(ChunkHolder holder) {
        ...
        CompletionStage completableFuture2 = ((CompletableFuture)completableFuture.thenApplyAsync(chunks -> chunks.mapLeft(cs -> (WorldChunk)cs.get(cs.size() / 2)), task -> this.mainExecutor.send(ChunkTaskPrioritySystem.createMessage(holder, task)))).thenApplyAsync(either -> either.ifLeft(chunk -> {
        ...
        ((CompletableFuture)completableFuture2).thenAcceptAsync(either -> either.ifLeft(chunk -> {
            ...
            this.getPlayersWatchingChunk(holder.getPos(), false).forEach(player -> this.sendChunkDataPackets((ServerPlayerEntity)player, mutableObject, (WorldChunk)chunk));
    ...
    public void updatePosition(ServerPlayerEntity player) {
        ...
        if (Math.abs(k - i) <= o * 2 && Math.abs(n - j) <= o * 2) {
            ...
            for (int t = p; t <= r; ++t) {
                for (int u = q; u <= s; ++u) {
                    boolean bl42 = ThreadedAnvilChunkStorage.isWithinDistance(t, u, k, n, this.watchDistance);
                    boolean bl5 = ThreadedAnvilChunkStorage.isWithinDistance(t, u, i, j, this.watchDistance);
                    this.sendWatchPackets(player, new ChunkPos(t, u), new MutableObject<ChunkDataS2CPacket>(), bl42, bl5);
                }
            }
        } else {
            boolean bl7;
            ...
            for (p = k - o; p <= k + o; ++p) {
                for (q = n - o; q <= n + o; ++q) {
                    if (!ThreadedAnvilChunkStorage.isWithinDistance(p, q, k, n, this.watchDistance)) continue;
                    bl6 = true;
                    bl7 = false;
                    this.sendWatchPackets(player, new ChunkPos(p, q), new MutableObject<ChunkDataS2CPacket>(), true, false);
                }
            }
            for (p = i - o; p <= i + o; ++p) {
                for (q = j - o; q <= j + o; ++q) {
                    if (!ThreadedAnvilChunkStorage.isWithinDistance(p, q, i, j, this.watchDistance)) continue;
                    bl6 = false;
                    bl7 = true;
                    this.sendWatchPackets(player, new ChunkPos(p, q), new MutableObject<ChunkDataS2CPacket>(), false, true);
                }
            }
        }
    }
