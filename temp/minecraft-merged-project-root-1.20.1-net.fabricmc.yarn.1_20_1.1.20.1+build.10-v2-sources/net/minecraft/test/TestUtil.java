...
public class TestUtil {
    ...
    public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions) {
        ...
        return map.entrySet().stream().flatMap(entry -> {
            ...
            return Streams.stream(Iterables.partition(collection, 100)).map(testFunctionsx -> new GameTestBatch(string + ":" + mutableInt.incrementAndGet(), ImmutableList.copyOf(testFunctionsx), consumer, consumer2));
    ...
    public static void clearTests(ServerWorld world, BlockPos pos, TestManager testManager, int radius) {
        ...
        BlockPos.stream(blockPos, blockPos2).filter(posx -> world.getBlockState((BlockPos)posx).isOf(Blocks.STRUCTURE_BLOCK)).forEach(posx -> {
            StructureBlockBlockEntity structureBlockBlockEntity = (StructureBlockBlockEntity)world.getBlockEntity((BlockPos)posx);
