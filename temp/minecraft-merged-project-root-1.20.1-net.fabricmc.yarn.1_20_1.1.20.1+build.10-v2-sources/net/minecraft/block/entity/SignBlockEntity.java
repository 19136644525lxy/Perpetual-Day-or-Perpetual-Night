...
extends BlockEntity {
    ...
    protected void writeNbt(NbtCompound nbt) {
        super.writeNbt(nbt);
        SignText.CODEC.encodeStart(NbtOps.INSTANCE, this.frontText).resultOrPartial(LOGGER::error).ifPresent(frontText -> nbt.put("front_text", (NbtElement)frontText));
        SignText.CODEC.encodeStart(NbtOps.INSTANCE, this.backText).resultOrPartial(LOGGER::error).ifPresent(backText -> nbt.put("back_text", (NbtElement)backText));
    ...
    public void readNbt(NbtCompound nbt) {
        ...
        if (nbt.contains("front_text")) {
            SignText.CODEC.parse(NbtOps.INSTANCE, nbt.getCompound("front_text")).resultOrPartial(LOGGER::error).ifPresent(signText -> {
                this.frontText = this.parseLines((SignText)signText);
            });
        }
        if (nbt.contains("back_text")) {
            SignText.CODEC.parse(NbtOps.INSTANCE, nbt.getCompound("back_text")).resultOrPartial(LOGGER::error).ifPresent(signText -> {
