...
implements Fertilizable {
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        if (PitcherCropBlock.isLowerHalf(state)) {
            return this.canPlantOnTop(world.getBlockState(pos.down()), world, pos.down()) && PitcherCropBlock.canPlaceAt(world, pos) && (state.get(AGE) < 3 || PitcherCropBlock.isUpperHalf(world.getBlockState(pos.up())));
        }
    ...
    private void tryGrow(ServerWorld world, BlockState state, BlockPos pos, int amount) {
        ...
        if (i >= 3) {
            ...
            world.setBlockState(blockPos, PitcherCropBlock.withWaterloggedState(world, pos, (BlockState)((BlockState)this.getDefaultState().with(AGE, i)).with(HALF, DoubleBlockHalf.UPPER)), Block.NOTIFY_ALL);
        }
    }
    ...
    private boolean canGrow(WorldView world, BlockPos pos, BlockState state, int age) {
        return !this.isFullyGrown(state) && PitcherCropBlock.canPlaceAt(world, pos) && (age < 3 || PitcherCropBlock.canGrowAt(world, pos.up()));
    }
