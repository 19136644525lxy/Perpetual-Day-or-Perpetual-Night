...
extends RenderPhase {
    ...
    private static final RenderLayer GLINT_TRANSLUCENT = RenderLayer.of("glint_translucent", VertexFormats.POSITION_TEXTURE, VertexFormat.DrawMode.QUADS, 256, MultiPhaseParameters.builder().program(TRANSLUCENT_GLINT_PROGRAM).texture(new RenderPhase.Texture(ItemRenderer.ITEM_ENCHANTMENT_GLINT, true, false)).writeMaskState(COLOR_MASK).cull(DISABLE_CULLING).depthTest(EQUAL_DEPTH_TEST).transparency(GLINT_TRANSPARENCY).texturing(GLINT_TEXTURING).target(ITEM_ENTITY_TARGET).build(false));
    ...
    private static final MultiPhase GUI = RenderLayer.of("gui", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS, 256, MultiPhaseParameters.builder().program(GUI_PROGRAM).transparency(TRANSLUCENT_TRANSPARENCY).depthTest(LEQUAL_DEPTH_TEST).build(false));
    private static final MultiPhase GUI_OVERLAY = RenderLayer.of("gui_overlay", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS, 256, MultiPhaseParameters.builder().program(GUI_OVERLAY_PROGRAM).transparency(TRANSLUCENT_TRANSPARENCY).depthTest(ALWAYS_DEPTH_TEST).writeMaskState(COLOR_MASK).build(false));
    private static final MultiPhase GUI_TEXT_HIGHLIGHT = RenderLayer.of("gui_text_highlight", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS, 256, MultiPhaseParameters.builder().program(GUI_TEXT_HIGHLIGHT_PROGRAM).transparency(TRANSLUCENT_TRANSPARENCY).depthTest(ALWAYS_DEPTH_TEST).colorLogic(OR_REVERSE).build(false));
    private static final MultiPhase GUI_GHOST_RECIPE_OVERLAY = RenderLayer.of("gui_ghost_recipe_overlay", VertexFormats.POSITION_COLOR, VertexFormat.DrawMode.QUADS, 256, MultiPhaseParameters.builder().program(GUI_GHOST_RECIPE_OVERLAY_PROGRAM).transparency(TRANSLUCENT_TRANSPARENCY).depthTest(BIGGER_DEPTH_TEST).writeMaskState(COLOR_MASK).build(false));
