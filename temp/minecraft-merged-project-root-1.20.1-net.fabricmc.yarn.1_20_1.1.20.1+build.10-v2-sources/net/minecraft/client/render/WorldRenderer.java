...
AutoCloseable {
    ...
    private boolean isOutsideViewDistance(BlockPos pos, ChunkBuilder.BuiltChunk chunk) {
        ...
        return !ThreadedAnvilChunkStorage.isWithinDistance(k, l = ChunkSectionPos.getSectionCoord(blockPos.getZ()), i, j, this.viewDistance - 3);
    }
    ...
    public void render(MatrixStack matrices, float tickDelta, long limitTime, boolean renderBlockOutline, Camera camera, GameRenderer gameRenderer, LightmapTextureManager lightmapTextureManager, Matrix4f projectionMatrix) {
        ...
        profiler.swap("sky");
        RenderSystem.setShader(GameRenderer::getPositionProgram);
        this.renderSky(matrices, projectionMatrix, tickDelta, camera, bl22, () -> BackgroundRenderer.applyFog(camera, BackgroundRenderer.FogType.FOG_SKY, g, bl22, tickDelta));
        profiler.swap("fog");
    ...
    private void renderLayer(RenderLayer renderLayer, MatrixStack matrices, double cameraX, double cameraY, double cameraZ, Matrix4f positionMatrix) {
        ...
        if (renderLayer == RenderLayer.getTranslucent()) {
            ...
            if (d * d + e * e + f * f > 1.0) {
                ...
                int l = 0;
                for (ChunkInfo chunkInfo : this.chunkInfos) {
                    if (l >= 15 || !bl && !chunkInfo.isOriginInSection(i, j, k) || !chunkInfo.chunk.scheduleSort(renderLayer, this.chunkBuilder)) continue;
                    ++l;
                }
            }
            this.client.getProfiler().pop();
    ...
    private void renderCapturedFrustumVertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, int planeNum) {
        vertexConsumer.vertex(matrix4f, this.capturedFrustumOrientation[planeNum].x(), this.capturedFrustumOrientation[planeNum].y(), this.capturedFrustumOrientation[planeNum].z()).color(0, 0, 0, 255).normal(0.0f, 0.0f, -1.0f).next();
    }
    ...
    private void renderCapturedFrustumFace(VertexConsumer vertexConsumer, Matrix4f matrix4f, int plane0, int plane1, int plane2, int plane3, int r, int g, int b) {
        float f = 0.25f;
        vertexConsumer.vertex(matrix4f, this.capturedFrustumOrientation[plane0].x(), this.capturedFrustumOrientation[plane0].y(), this.capturedFrustumOrientation[plane0].z()).color((float)r, (float)g, (float)b, 0.25f).next();
        vertexConsumer.vertex(matrix4f, this.capturedFrustumOrientation[plane1].x(), this.capturedFrustumOrientation[plane1].y(), this.capturedFrustumOrientation[plane1].z()).color((float)r, (float)g, (float)b, 0.25f).next();
        vertexConsumer.vertex(matrix4f, this.capturedFrustumOrientation[plane2].x(), this.capturedFrustumOrientation[plane2].y(), this.capturedFrustumOrientation[plane2].z()).color((float)r, (float)g, (float)b, 0.25f).next();
        vertexConsumer.vertex(matrix4f, this.capturedFrustumOrientation[plane3].x(), this.capturedFrustumOrientation[plane3].y(), this.capturedFrustumOrientation[plane3].z()).color((float)r, (float)g, (float)b, 0.25f).next();
    }
