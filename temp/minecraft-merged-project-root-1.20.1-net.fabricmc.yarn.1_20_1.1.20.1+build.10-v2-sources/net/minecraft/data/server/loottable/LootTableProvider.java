...
implements DataProvider {
    ...
    public CompletableFuture<?> run(DataWriter writer) {
        final HashMap<Identifier, LootTable> map = Maps.newHashMap();
        Object2ObjectOpenHashMap map2 = new Object2ObjectOpenHashMap();
        this.lootTypeGenerators.forEach(lootTypeGenerator -> lootTypeGenerator.provider().get().accept((identifier, builder) -> {
            Identifier identifier2 = map2.put(RandomSequence.createSeed(identifier), identifier);
            if (identifier2 != null) {
                Util.error("Loot table random sequence seed collision on " + identifier2 + " and " + identifier);
            }
            builder.randomSequenceId((Identifier)identifier);
            if (map.put((Identifier)identifier, builder.type(lootTypeGenerator.paramSet).build()) != null) {
