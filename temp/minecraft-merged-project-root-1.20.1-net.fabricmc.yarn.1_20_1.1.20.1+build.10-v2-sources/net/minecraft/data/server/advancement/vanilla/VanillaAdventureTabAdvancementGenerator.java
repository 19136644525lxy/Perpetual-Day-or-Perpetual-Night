...
implements AdvancementTabGenerator {
    ...
    public void accept(RegistryWrapper.WrapperLookup lookup, Consumer<Advancement> exporter) {
        ...
        VanillaAdventureTabAdvancementGenerator.requireAllExclusiveTrimmedArmor(Advancement.Builder.create()).parent(advancement12).display(new ItemStack(Items.SILENCE_ARMOR_TRIM_SMITHING_TEMPLATE), (Text)Text.translatable("advancements.adventure.trim_with_all_exclusive_armor_patterns.title"), (Text)Text.translatable("advancements.adventure.trim_with_all_exclusive_armor_patterns.description"), null, AdvancementFrame.CHALLENGE, true, true, false).build(exporter, "adventure/trim_with_all_exclusive_armor_patterns");
        Advancement.Builder.create().parent(advancement).display(Items.CHISELED_BOOKSHELF, (Text)Text.translatable("advancements.adventure.read_power_from_chiseled_bookshelf.title"), (Text)Text.translatable("advancements.adventure.read_power_from_chiseled_bookshelf.description"), null, AdvancementFrame.TASK, true, true, false).criteriaMerger(CriterionMerger.OR).criterion("chiseled_bookshelf", VanillaAdventureTabAdvancementGenerator.requirePlacedBlockReadByComparator(Blocks.CHISELED_BOOKSHELF)).criterion("comparator", VanillaAdventureTabAdvancementGenerator.requirePlacedComparatorReadingBlock(Blocks.CHISELED_BOOKSHELF)).build(exporter, "adventure/read_power_of_chiseled_bookshelf");
    }
    ...
    private static CriterionConditions requirePlacedComparatorReadingBlock(Block block) {
        LootCondition.Builder[] builders = (LootCondition.Builder[])ComparatorBlock.FACING.getValues().stream().map(facing -> {
            ...
            LootCondition.Builder builder3 = LocationCheckLootCondition.builder(LocationPredicate.Builder.create().block(BlockPredicate.Builder.create().blocks(block).build()), new BlockPos(facing.getVector()));
