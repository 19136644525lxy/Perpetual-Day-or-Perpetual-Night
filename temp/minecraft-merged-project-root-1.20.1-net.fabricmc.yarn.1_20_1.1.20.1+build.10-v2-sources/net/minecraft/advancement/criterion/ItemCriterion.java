...
extends AbstractCriterion<Conditions> {
    ...
    public Conditions conditionsFromJson(JsonObject jsonObject, LootContextPredicate lootContextPredicate, AdvancementEntityPredicateDeserializer advancementEntityPredicateDeserializer) {
        LootContextPredicate lootContextPredicate2 = LootContextPredicate.fromJson("location", advancementEntityPredicateDeserializer, jsonObject.get("location"), LootContextTypes.ADVANCEMENT_LOCATION);
        if (lootContextPredicate2 == null) {
            throw new JsonParseException("Failed to parse 'location' field");
        }
        return new Conditions(this.id, lootContextPredicate, lootContextPredicate2);
    }

    public void trigger(ServerPlayerEntity player, BlockPos pos, ItemStack stack) {
        ServerWorld serverWorld = player.getServerWorld();
        BlockState blockState = serverWorld.getBlockState(pos);
        LootContextParameterSet lootContextParameterSet = new LootContextParameterSet.Builder(serverWorld).add(LootContextParameters.ORIGIN, pos.toCenterPos()).add(LootContextParameters.THIS_ENTITY, player).add(LootContextParameters.BLOCK_STATE, blockState).add(LootContextParameters.TOOL, stack).build(LootContextTypes.ADVANCEMENT_LOCATION);
    ...
    extends AbstractCriterionConditions {
        ...
        public static Conditions createPlacedBlock(Block block) {
            ...
            return new Conditions(Criteria.PLACED_BLOCK.id, LootContextPredicate.EMPTY, lootContextPredicate);
        }

        public static Conditions createPlacedBlock(LootCondition.Builder ... locationConditions) {
        ...
        private static Conditions create(LocationPredicate.Builder location, ItemPredicate.Builder item, Identifier id) {
            ...
            return new Conditions(id, LootContextPredicate.EMPTY, lootContextPredicate);
        }

        public static Conditions createItemUsedOnBlock(LocationPredicate.Builder location, ItemPredicate.Builder item) {
            return Conditions.create(location, item, Criteria.ITEM_USED_ON_BLOCK.id);
        }

        public static Conditions createAllayDropItemOnBlock(LocationPredicate.Builder location, ItemPredicate.Builder item) {
            return Conditions.create(location, item, Criteria.ALLAY_DROP_ITEM_ON_BLOCK.id);
        }
